          ...
          <table class="q-table">
            <thead>
              <tr class="q-tr">
                ...
                <th class="text-right">
                  <span class="text-bold text-blue-grey-10">{{$t('product.quantityStore')}}</span>
                </th>
                <th class="text-right">
            ...
            <tbody>
              ...
              <tr>
                <td colspan="4" class="text-left text-bold">{{$t('product.total')}}</td>
                <td class="text-right text-bold">
                  {{total.quantities.NumberFormat($store.getters.language)}}</td>
                <td class="text-right text-bold">
                  {{total.prices.NumberFormat($store.getters.language)}}
                </td>
              </tr>
              <tr>
                <td colspan="5" class="text-left text-bold">{{$t('product.tax')}}</td>
                <td class="text-right text-bold">
                  {{total.taxes.NumberFormat($store.getters.language)}}
                </td>
              </tr>
              <tr>
                <td colspan="5" class="text-left text-bold">{{$t('product.totalAmount')}}</td>
                <td class="text-right text-bold">
                  {{(total.prices+total.taxes).NumberFormat($store.getters.language)}}
                </td>
              </tr>
            </tbody>
          </table>
