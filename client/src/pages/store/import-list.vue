    ...
    <template v-slot:top="props">
      <div class="col-12 row">
        ...
        <div class="col-xs-12 col-sm-auto self-center row">
          <q-btn flat round dense icon="add" color="blue" @click="$router.push('import')">
              ...
              <q-item clickable v-close-popup @click="onPrinting('details')">
                <q-item-section>
                  <q-item-label>
                    ...
                    <tm-printer ref="refPrinterDetails" hidden :title="`${$t('product.importBallotDetails')}-${props.row._id}`">
              ...
              <q-item clickable v-close-popup @click="onPrinting('total')">
                <q-item-section>
                  <q-item-label>
                    ...
                    <tm-printer ref="refPrinterTotal" hidden :title="`${$t('product.importBallotDetails')}-${props.row._id}`">
                      <template v-slot:content>
                        <view-total :title="$t('product.importBallotTotal')" :total="props.row" color="blue"
  ...
  <q-dialog v-model="isDialogDetails" :maximized="isMaximizedView">
    <q-card flat :style="{minWidth:'60%'}">
      <q-toolbar>
        ...
        <q-toolbar-title>{{$t('product.importBallotDetails')}}</q-toolbar-title>
...
export default defineComponent({
  ...
  setup () {
    ...
    const onFetch = (props) => {
      ...
      $store.dispatch('store/importGet', props.pagination).then(x => {
    ...
    const LoadDetails = (id) => {
      $store.dispatch('store/importGetDetail', { key: id }).then((x) => {
