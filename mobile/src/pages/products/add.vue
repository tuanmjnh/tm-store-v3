    ...
    <q-card-section class="q-pa-none">
      <q-form ref="form">
        ...
        <q-scroll-area style="height:calc(100vh - 99px)">
          ...
          <q-tab-panel name="images" id="tab-images" class="q-pa-none">
            <tm-fileList ref="refTMFileList" v-model="data.images" v-model:selected="selectedFileList" v-model:view-type="viewType" :multiple="true"
                         :lblConfirmTitle="$t('messageBox.warning')" :lblConfirmContent="$t('messageBox.delete')" :lblOk="$t('global.accept')"
                         :lblCancel="$t('global.cancel')">
              <template v-slot:tool-bar>
                <q-btn round dense flat icon="cloud_upload" color="primary" @click="isDialogUpload=!isDialogUpload" />
                <!-- <q-popup-proxy>
          ...
          <q-tab-panel name="qrcode" id="tab-qrcode">
    ...
    <q-card>
      <q-card-section class="q-pl-md q-pr-md q-pt-none q-pb-none">
        ...
        <tm-fileManager lblAccept="" :lblConfirmTitle="$t('messageBox.warning')" :lblConfirmContent="$t('messageBox.delete')"
                        ...
                        :url="$store.state.app.apiUpload" @onAccept="onAccept" :multiple="true" path="products"
                        :headers="[{name:'Upload-Path',value:'products'},{ name:'Upload-Rename',value:true},{name:'x-access-token',value:`Bearer ${$store.state.auth.token}`}]">
...
import { getExtension, getNameFilePath } from '../../../../global/utils/extension'
  ...
  setup (props, { emit }) {
    ...
    const generateImages = (images) => {
      if (images && images.length) {
        ...
        const rs = []
        _images.forEach(e => {
          const type = getExtension(e)
          rs.push({
            name: getNameFilePath(e),
            url: e,
            type: type,
            size: 0,
            extension: type
    ...
    const onReset = () => {
      return new Promise((resolve, reject) => {
        ...
        data.value.images = generateImages(data.value.images)
    ...
    return {
      ...
      onSubmit () {
        form.value.validate().then(async (valid) => {
          if (valid) {
            ...
            if (data.value.images && data.value.images.length) {
              data.value.images = data.value.images.map(x => x.url)
            } else {
              data.value.images = null
            }
            if ($store.state.products.item._id) {
      ...
      onUploaded (val) {
        ...
        if (data.value.images) {
          val.forEach(e => {
            if (data.value.images.findIndex(x => x.url === e.url) < 0)
              data.value.images.push(e)
          })
        } else {
          data.value.images = val
        }
      },
      onAccept (val) {
